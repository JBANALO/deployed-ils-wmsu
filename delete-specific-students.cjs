...
async function deleteSpecificStudents() {
  ...
  try {
    ...
    // List of students to delete (firstName, lastName)
    ...
    for (const [firstName, lastName] of studentsToDelete) {
      const result = await connection.query(
        'DELETE FROM students WHERE first_name = ? AND last_name = ?',
        [firstName, lastName]
      );
    ...
    // Also delete from users table these students
    for (const [firstName, lastName] of studentsToDelete) {
      await connection.query(
        'DELETE FROM users WHERE first_name = ? AND last_name = ? AND role = ?',
    ...
    console.log(`📊 Deleted from students: ${deletedCount}`);
