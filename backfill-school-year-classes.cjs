...
(async () => {
  ...
  const run = async () => {
    console.log('➡️ Starting backfill for classes/subject_teachers school_year_id');
    ...
    await ensureColumn('classes', 'school_year_id', 'idx_classes_school_year');
    await ensureColumn('subject_teachers', 'school_year_id', 'idx_subject_teachers_school_year');
    ...
    await backfill('classes');
    await backfill('subject_teachers');
