...
export default function TeacherBulkImportModal({ isOpen, onClose, onSuccess }) {
  ...
  const handleDownloadTemplate = () => {
    ...
    const headers = ['No.', 'First Name', 'Middle Name', 'Last Name', 'Username', 'Email'];
            ...
            >
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <p className="text-blue-800 text-sm">
                  ...
                  Role, Grade Level, and Section will be assigned later by the admin via the Assign Adviser page.
                </p>
              </div>
              ...
              <div>
                ...
                <div className="bg-gray-50 p-4 rounded-lg text-sm font-mono text-gray-700 overflow-x-auto">
                  <div>No., First Name, Middle Name, Last Name, Username, Email, Password</div>
                  <div className="text-xs text-gray-600 mt-2">
                    ...
                    • Middle Name<br />
                    • Username (auto-generated from name)<br />
                    • Email (auto-generated as firstname.lastname@wmsu.edu.ph)<br />
                    <br />
                    <span className="text-blue-600">• Role, Grade Level, Section, and Subjects are assigned later via the Assign Adviser page</span>
                  </div>
                </div>
              </div>
                  ...
                  <tbody>
                    {csvData.map((teacher, idx) => (
                      <tr key={idx} className="border-t hover:bg-gray-50">
                        <td className="px-4 py-2">{teacher.firstName} {teacher.middleName ? `${teacher.middleName} ` : ''}{teacher.lastName}</td>
                        <td className="px-4 py-2 text-sm text-gray-600">{teacher.email}</td>
                        <td className="px-4 py-2 text-sm text-gray-600">{teacher.username}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                ...
                {importResults.map((result, idx) => (
                  ...
                  >
                    ...
                    <span className="text-xs px-2 py-1 rounded bg-gray-100">
                      {result.status === 'imported' ? 'New' : result.status === 'updated' ? 'Updated' : 'Error'}
                    </span>
                  </div>
                ))}
              </div>
